...
public class HuaweiRIL2 extends RIL implements CommandsInterface {
    ...
    protected DataCallState getDataCallState(Parcel p, int version) {
        DataCallState dataCall = new DataCallState();
            ...
            dataCall.suggestedRetryTime = p.readInt();//-1;
    ...
    responseSetupDataCall(Parcel p) {
		...
		if (RILJ_LOGV) riljLog("DataCall status=" + dataCall.status);
		dataCall.suggestedRetryTime = p.readInt();//-1;
		if (RILJ_LOGV) riljLog("DataCall suggestedRetryTime=" + dataCall.suggestedRetryTime);
		dataCall.cid = p.readInt(); //cid
		if (RILJ_LOGV) riljLog("DataCall cid=" + dataCall.cid);
		dataCall.active = p.readInt(); //active
		if (RILJ_LOGV) riljLog("DataCall active=" + dataCall.active);
         dataCall.type = p.readString(); //type;
		if (RILJ_LOGD) riljLog("responseSetupDataCall type = " + dataCall.type);
		dataCall.ifname = p.readString(); //ifname mLastDataIface;
		if (RILJ_LOGV) riljLog("DataCall ifname=" + dataCall.ifname);
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
            ...
            case RIL_REQUEST_ISIM_AUTHENTICATION: ret = responseInts(p); break; 
            case RIL_REQUEST_ACKNOWLEDGE_INCOMING_GSM_SMS_WITH_PDU: ret = responseStrings(p); break; 
            case RIL_REQUEST_STK_SEND_ENVELOPE_WITH_STATUS: ret = responseInts(p);  break; 
    ...
    processUnsolicited (Parcel p) {
        ...
        switch(response) {
            ...
            case 1037: ret = responseInts(p); break; 
            case 1038: ret = responseVoid(p); break; 
			case 1041: ret =  responseInts(p); break; 
	...
	public void setTransmitPower(int powerLevel, Message result) {
        RILRequest rr = RILRequest.obtain(117, result);
